...
import getLiveGames from './app/getter/getLiveGames';
import getUserWithAvatar from './app/getter/getUserWithAvatar';
...
export default async function (server: any) {
  ...
  io.on('connection', (socket: any) => {
    ...
    socket.on('join-channel', async ({channelSlug}: any, cb: any) => {
      ...
      if (token) {
        ...
        if (uid) {
          const User = await getUserWithAvatar(uid);
    ...
    socket.on(
      'send-message',
      async (
        {channelSlug, text, User, repliedTo, chatID, lockerRoomSlug, Media, MentionedUserIDs, gif}: any,
        cb: any
      ) => {
        emitLogs({
        ...
        let error = null;
        await sendMessage(null, userID, channelSlug, text, chatID, repliedToChatID, Media, MentionedUserIDs, gif).catch(
          e => {
            error = e;
          }
        );
    ...
    socket.on(
      'edit-message',
      async ({channelSlug, chatID, text, repliedToChatID, Media, MentionedUserIDs, gif}: any, cb: any) => {
        if (token) {
          let error = null;
          const {uid} = await auth.validateToken(token, global.firebaseBE);
          if (!uid) return;
          editMessage(chatID, text, repliedToChatID, Media, uid, MentionedUserIDs, gif).catch(e => {
            error = e;
          });
          if (!error) {
  ...
  // Create a cron job that sends live games
  const CYCLE_TEN_SEC = '*/10 * * * * *';
  new CronJob(CYCLE_TEN_SEC, async () => {
    try {
      const liveGames = await getLiveGames();
      const call = 'live-games',
        payload = {games: liveGames};
      io.emit('live-games', {
        call,
        payload,
      });
    ...
    (
      ...
      return {
        ...
        User: {
          ...
          Avatar: {
            PhotoURL: user?.Avatar?.objectID ? getDerivedPhotoURL(user.Avatar.objectID) : null,
...
async function getMediaPhotoURLs(Media: Array<any>, type: string) {
  ...
  for (const item of Media) {
    item.PhotoURL = getDerivedPhotoURL(item.objectID, false, size);
    item.isSport = false;
    MediaList.push(item);
  }
  return MediaList;
