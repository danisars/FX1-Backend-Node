...
export const MessageMutations = mutationField(t => {
  t.nonNull.field('sendMessage', {
    ...
    resolve: async (source, {channelSlug, text, chatID, repliedToChatID, Media, MentionedUserIDs, gif}, context) => {
      const {uid} = context;
      // 1. Require a logged in user.
      requireLoggedIn(uid);
      // 2. Require a user account.
  ...
  t.nonNull.field('editMessage', {
    ...
    resolve: async (source, {chatID, text, repliedToChatID, Media, MentionedUserIDs, gif}, context) => {
      const {uid} = context;
      return await editMessage(chatID, text, repliedToChatID, Media, uid, MentionedUserIDs, gif);
    },
  });
