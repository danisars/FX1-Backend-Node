...
export const ChannelGroup = objectType({
  ...
  definition(t) {
    ...
    t.list.field('Channels', {
      type: 'Channel',
      resolve: async ({id, lockerRoomID}, args, {uid, userID}) => {
        // get userID directly from database
        const currentUser = await User.findOne({uid}).exec();
        const hasPrivate = await UserRole.findOne({lockerRoomID, groupType: 'Channel', userID: currentUser?.id}).exec();
        const filter = {$or: [{...channelFilter, channelGroupID: id, type: 'Public'}, {_id: hasPrivate?.groupID}]};
        return await Channel.find(filter).exec();
      },
    });
  },
});
