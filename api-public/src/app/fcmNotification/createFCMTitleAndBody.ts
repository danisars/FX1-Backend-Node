...
export default async function (
  ...
  let photoURL;
  if (Media) {
    photoURL = getPhotoURL(Media[0].objectID, Media[0].objectType, false, '1024');
  }
  if (text) {
...
export const createInvitePushNotificationTitleAndBody = async (gameID: string, actorID: string) => {
  const game = await gameIDExists(gameID.toString());
  const actor = await userIDExists(actorID.toString());

  const title = 'FX1 Notification';
  const body = `Come join me to follow the ${game.team1DisplayName} vs ${game.team2DisplayName} game on FX1`;
  return {
    title,
    body,
  };
