...
import createFCMTitleAndBody, {
  createReminderPushNoficationTitleAndBody,
  createInvitePushNotificationTitleAndBody,
} from './createFCMTitleAndBody';
...
export const sendInvitePushNotification = async (topic: string, data: any) => {
  const {title, body} = await createInvitePushNotificationTitleAndBody(data.gameID, data.actorID);
  return sendMessage(
    topic,
    {
      title,
